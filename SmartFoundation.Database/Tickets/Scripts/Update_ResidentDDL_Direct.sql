-- Direct script to drop and recreate TicketDL with updated ResidentDDL
-- Execute this script directly in SSMS

USE [DATACORETi]
GO

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Tickets].[TicketDL]') AND type in (N'P', N'PC'))
DROP PROCEDURE [Tickets].[TicketDL]
GO
...
PRINT 'TicketDL recreated successfully with updated ResidentDDL!';
