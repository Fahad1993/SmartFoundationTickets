...
# Do NOT use set -e here — a single gh failure should not abort the whole run.
# Individual item failures are tracked and reported in the summary at the end.

# ---------------------------------------------------------------------------
# Authentication
# ---------------------------------------------------------------------------
# This script requires a GitHub fine-grained personal access token (PAT) with
# "Projects: Read and write" permission for the project owner.
#
# Classic PATs with only "repo" and "project" scopes will fail with:
#   "your authentication token is missing required scopes [read:org read:discussion]"
# Use a fine-grained PAT instead.
#
# How to provide the token (choose one):
#   1. Export before running:
#        export GH_TOKEN=github_pat_YOUR_TOKEN_HERE
#        bash populate_kanban.sh
#   2. Inline when running:
#        GH_TOKEN=github_pat_YOUR_TOKEN_HERE bash populate_kanban.sh
#   3. Store in .env and load it:
#        echo 'GH_TOKEN=github_pat_YOUR_TOKEN_HERE' >> .env
#        bash populate_kanban.sh  # script auto-loads .env
#
...
# Load .env if it exists and GH_TOKEN is not already set.
# Uses a line-by-line approach to avoid executing arbitrary code.
if [ -z "${GH_TOKEN}" ] && [ -f "$(dirname "$0")/.env" ]; then
  while IFS='=' read -r key value; do
    # Skip blank lines and comments
    case "$key" in
      ''|\#*) continue ;;
    esac
    if [ "$key" = "GH_TOKEN" ] && [ -n "$value" ]; then
...
if [ -z "${GH_TOKEN}" ]; then
  ...
  echo ""
  echo "Please generate a fine-grained personal access token at:"
  ...
  echo ""
  echo "Then run:"
  echo "  GH_TOKEN=github_pat_YOUR_TOKEN_HERE bash populate_kanban.sh"
  echo ""
  exit 1
fi
...
# Failure tracking — records which items could not be created
FAILED_ITEMS=0
TOTAL_ITEMS=0

# Wrapper that counts successes and failures
run_item() {
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-00] Create [Tickets] schema in MSSQL" --body "**Spec:** 00 - Project Setup
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-00] Create Database directory structure in repository" --body "**Spec:** 00 - Project Setup
**Category:** Docs
**Dependencies:** None
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Create [Tickets].[TicketStatus] lookup table" --body "**Spec:** 01 - Lookup Foundations
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Create [Tickets].[TicketClass] lookup table" --body "**Spec:** 01 - Lookup Foundations
**Category:** DB-Structure
**Dependencies:** Spec-00 (schema)
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Create [Tickets].[Priority] lookup table" --body "**Spec:** 01 - Lookup Foundations
**Category:** DB-Structure
**Dependencies:** Spec-00 (schema)
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Create [Tickets].[RequesterType] lookup table" --body "**Spec:** 01 - Lookup Foundations
**Category:** DB-Structure
**Dependencies:** Spec-00 (schema)
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Create [Tickets].[PauseReason] lookup table" --body "**Spec:** 01 - Lookup Foundations
**Category:** DB-Structure
**Dependencies:** Spec-00 (schema)
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Create [Tickets].[ArbitrationReason] lookup table" --body "**Spec:** 01 - Lookup Foundations
**Category:** DB-Structure
**Dependencies:** Spec-00 (schema)
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Create [Tickets].[ClarificationReason] lookup table" --body "**Spec:** 01 - Lookup Foundations
**Category:** DB-Structure
**Dependencies:** Spec-00 (schema)
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Create [Tickets].[QualityReviewResult] lookup table" --body "**Spec:** 01 - Lookup Foundations
**Category:** DB-Structure
**Dependencies:** Spec-00 (schema)
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Seed all lookup tables with initial values" --body "**Spec:** 01 - Lookup Foundations
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-01] Test lookup uniqueness and seed integrity" --body "**Spec:** 01 - Lookup Foundations
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Create [Tickets].[Service] master table" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Create [Tickets].[ServiceRoutingRule] master table" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Create [Tickets].[ServiceSLAPolicy] master table" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Create [Tickets].[ServiceCatalogSuggestion] master table" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-Structure
**Dependencies:** Spec-02 Service table, Spec-01 Priority
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Implement [Tickets].[ServiceSP] - service CRUD actions" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Implement [Tickets].[ServiceSP] - routing rule actions" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Implement [Tickets].[ServiceSP] - SLA policy upsert action" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Implement [Tickets].[ServiceSP] - suggestion approval/rejection" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Create [Tickets].[V_ServiceFullDefinition] view" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Create [Tickets].[ServiceDL] data layer procedure" --body "**Spec:** 02 - Service Catalogue
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-02] Test service catalogue foundation flows" --body "**Spec:** 02 - Service Catalogue
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-02] Build service catalogue admin list screen" --body "**Spec:** 02 - Service Catalogue
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-02] Build service create/edit screen" --body "**Spec:** 02 - Service Catalogue
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-02] Build routing rule maintenance screen" --body "**Spec:** 02 - Service Catalogue
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-02] Build SLA policy maintenance and suggestion review screens" --body "**Spec:** 02 - Service Catalogue
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-03] Create [Tickets].[Ticket] transaction table" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-03] Create [Tickets].[TicketHistory] history table" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-03] Implement INSERT_TICKET action in [Tickets].[TicketSP]" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-03] Implement requester type validation and root ticket initialization" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-03] Create [Tickets].[V_TicketFullDetails] and [V_TicketLastAction] views" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-03] Create [Tickets].[TicketDL] - basic detail and list actions" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-03] Test core ticket creation flow" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-03] Build ticket creation screen" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-03] Build ticket details screen" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-03] Build basic ticket list screen" --body "**Spec:** 03 - Core Ticket Backbone
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-04] Implement ASSIGN_TICKET in TicketSP" --body "**Spec:** 04 - Assignment and Work Start
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-04] Implement MOVE_TO_IN_PROGRESS in TicketSP" --body "**Spec:** 04 - Assignment and Work Start
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-04] Implement REJECT_TO_SUPERVISOR in TicketSP" --body "**Spec:** 04 - Assignment and Work Start
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-04] Implement assignment eligibility validation" --body "**Spec:** 04 - Assignment and Work Start
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-04] Extend TicketDL for queue inbox reads" --body "**Spec:** 04 - Assignment and Work Start
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-04] Test assignment and work start flow" --body "**Spec:** 04 - Assignment and Work Start
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-04] Build scope-based queue inbox screen" --body "**Spec:** 04 - Assignment and Work Start
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-04] Build assignment action UI" --body "**Spec:** 04 - Assignment and Work Start
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-04] Build start work action UI" --body "**Spec:** 04 - Assignment and Work Start
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-04] Build reject-to-supervisor action UI" --body "**Spec:** 04 - Assignment and Work Start
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-05] Create [Tickets].[ClarificationRequest] transaction table" --body "**Spec:** 05 - Clarification Flow
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-05] Implement [Tickets].[ClarificationSP] - all actions" --body "**Spec:** 05 - Clarification Flow
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-05] Link blocking clarification to pause sessions" --body "**Spec:** 05 - Clarification Flow
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-05] Test clarification flow" --body "**Spec:** 05 - Clarification Flow
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-05] Build clarification request form" --body "**Spec:** 05 - Clarification Flow
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-05] Build clarification response form" --body "**Spec:** 05 - Clarification Flow
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-05] Show clarification state in ticket details" --body "**Spec:** 05 - Clarification Flow
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-06] Create [Tickets].[ArbitrationCase] transaction table" --body "**Spec:** 06 - Arbitration Flow
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-06] Implement [Tickets].[ArbitrationSP] - all actions" --body "**Spec:** 06 - Arbitration Flow
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-06] Create [Tickets].[ArbitrationDL] data layer procedure" --body "**Spec:** 06 - Arbitration Flow
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-06] Test arbitration flow" --body "**Spec:** 06 - Arbitration Flow
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-06] Build arbitration inbox screen" --body "**Spec:** 06 - Arbitration Flow
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-06] Build arbitration decision screen" --body "**Spec:** 06 - Arbitration Flow
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-06] Show arbitration status in ticket details" --body "**Spec:** 06 - Arbitration Flow
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-07] Implement CREATE_CHILD_TICKET in TicketSP" --body "**Spec:** 07 - Parent-Child Ticketing
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-07] Implement parent/root inheritance logic and extend TicketDL for tree" --body "**Spec:** 07 - Parent-Child Ticketing
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-07] Test parent-child ticket flow" --body "**Spec:** 07 - Parent-Child Ticketing
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-07] Build child ticket creation UI" --body "**Spec:** 07 - Parent-Child Ticketing
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-07] Build parent-child tree visualization" --body "**Spec:** 07 - Parent-Child Ticketing
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-08] Create [Tickets].[TicketPauseSession] transaction table" --body "**Spec:** 08 - Blocking and Pause Sessions
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-08] Implement PAUSE_TICKET in TicketSP" --body "**Spec:** 08 - Blocking and Pause Sessions
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-08] Implement RESUME_TICKET in TicketSP" --body "**Spec:** 08 - Blocking and Pause Sessions
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-08] Implement parent blocking rules" --body "**Spec:** 08 - Blocking and Pause Sessions
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-08] Test pause and resume flow" --body "**Spec:** 08 - Blocking and Pause Sessions
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-08] Build pause and resume action UI" --body "**Spec:** 08 - Blocking and Pause Sessions
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-08] Display active blocking reason in ticket details" --body "**Spec:** 08 - Blocking and Pause Sessions
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-09] Create [Tickets].[TicketSLA] transaction table" --body "**Spec:** 09 - SLA Engine
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-09] Create [Tickets].[TicketSLAHistory] history table" --body "**Spec:** 09 - SLA Engine
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-09] Implement [Tickets].[TicketSLASP] - all actions" --body "**Spec:** 09 - SLA Engine
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-09] Implement SLA initialization from ServiceSLAPolicy" --body "**Spec:** 09 - SLA Engine
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-09] Implement SLA pause/resume integration with ticket pausing" --body "**Spec:** 09 - SLA Engine
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-09] Create [Tickets].[V_TicketCurrentSLA] view" --body "**Spec:** 09 - SLA Engine
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-09] Test SLA engine" --body "**Spec:** 09 - SLA Engine
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-09] Show SLA timers and breach markers in ticket UI" --body "**Spec:** 09 - SLA Engine
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-09] Show SLA state in queue list screens" --body "**Spec:** 09 - SLA Engine
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-10] Create [Tickets].[QualityReview] transaction table" --body "**Spec:** 10 - Quality Review
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-10] Implement RESOLVE_OPERATIONALLY in TicketSP" --body "**Spec:** 10 - Quality Review
**Category:** DB-SP
**Dependencies:** Spec-03 TicketSP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-10] Implement [Tickets].[QualityReviewSP] - all actions" --body "**Spec:** 10 - Quality Review
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-10] Implement CLOSE_TICKET and REOPEN_TICKET in TicketSP" --body "**Spec:** 10 - Quality Review
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-10] Implement final closure validation logic" --body "**Spec:** 10 - Quality Review
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-10] Test quality review and final closure flow" --body "**Spec:** 10 - Quality Review
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-10] Build quality review inbox screen" --body "**Spec:** 10 - Quality Review
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-10] Build final closure approval and return-for-correction UI" --body "**Spec:** 10 - Quality Review
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-10] Display two-stage closure state in ticket details" --body "**Spec:** 10 - Quality Review
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-11] Create [Tickets].[CatalogRoutingChangeLog] history table" --body "**Spec:** 11 - Catalogue Learning
**Category:** DB-Structure
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-11] Implement suggestion-to-service creation flow" --body "**Spec:** 11 - Catalogue Learning
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-11] Log routing corrections from arbitration historically" --body "**Spec:** 11 - Catalogue Learning
**Category:** DB-SP
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-11] Test catalogue learning and routing correction" --body "**Spec:** 11 - Catalogue Learning
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-11] Build service suggestion approval UI" --body "**Spec:** 11 - Catalogue Learning
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-11] Build routing correction review UI" --body "**Spec:** 11 - Catalogue Learning
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-11] Show routing change history for a service" --body "**Spec:** 11 - Catalogue Learning
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-12] Create [Tickets].[V_TicketInboxByScope] view" --body "**Spec:** 12 - Reporting
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-12] Create [Tickets].[V_TicketArbitrationInbox] and [V_TicketQualityInbox] views" --body "**Spec:** 12 - Reporting
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-12] Create [Tickets].[DashboardDL] data layer procedure" --body "**Spec:** 12 - Reporting
**Category:** DB-DL-View
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-12] Test dashboard and reporting queries" --body "**Spec:** 12 - Reporting
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-12] Build organizational leadership dashboard" --body "**Spec:** 12 - Reporting
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-12] Build quality and monitoring dashboard widgets" --body "**Spec:** 12 - Reporting
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-12] Build overdue tickets report screen" --body "**Spec:** 12 - Reporting
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-12] Build service frequency and workload report" --body "**Spec:** 12 - Reporting
**Category:** UI
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-13] Create structural integrity test suite" --body "**Spec:** 13 - Testing
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-13] Create business scenario test suite (10 scenarios)" --body "**Spec:** 13 - Testing
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-13] Create SLA-specific test suite" --body "**Spec:** 13 - Testing
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-13] Create reporting validation test suite" --body "**Spec:** 13 - Testing
**Category:** Test
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-14] Create master deployment script (Deploy_All.sql)" --body "**Spec:** 14 - Documentation
**Category:** Docs
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-14] Create rollback script (Rollback_All.sql)" --body "**Spec:** 14 - Documentation
**Category:** Docs
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-14] Document stored procedure API reference" --body "**Spec:** 14 - Documentation
**Category:** Docs
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-14] Document business rule matrix (BR-01 to BR-20)" --body "**Spec:** 14 - Documentation
**Category:** Docs
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-14] Create UI page map, navigation structure, and component library specs" --body "**Spec:** 14 - Documentation
**Category:** Docs
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-15] Set up frontend project scaffolding and configuration" --body "**Spec:** 15 - Frontend Infrastructure
**Category:** Setup
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Frontend][Spec-15] Implement API service layer for all backend endpoints" --body "**Spec:** 15 - Frontend Infrastructure
**Category:** Setup
...
run_item gh project item-create ${PROJECT} --owner "${OWNER}" --title "[Backend][Spec-15] Create API endpoint layer and DB connection utility" --body "**Spec:** 15 - Backend Infrastructure
**Category:** Setup
...
echo "========================================="
echo ""
echo " Results: $((TOTAL_ITEMS - FAILED_ITEMS)) / ${TOTAL_ITEMS} items created successfully."
if [ "${FAILED_ITEMS}" -gt 0 ]; then
  echo " WARNING: ${FAILED_ITEMS} item(s) failed. Re-run the script to retry — duplicate"
  echo "          titles on the same board are accepted by GitHub Projects."
  echo ""
  exit 1
fi